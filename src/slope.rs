...
impl Pipeline {
    ...
    pub async fn get(
        ...
        if !output.exists() {
            ...
            let tile_center = tile::square_to_geodetic(&tile::tile_to_square(
                zoom,
                x as f64 + 0.5,
                y as f64 + 0.5,
            ));
...
pub fn slope(input: path::PathBuf, scale: f64) -> Result<path::PathBuf> {
...
mod tests {
    ...
    #[test]
    fn test_slope() {
        let output = slope(path::Path::new("/tmp/srtm_13_03.tif").to_path_buf(), 1.0).unwrap();
